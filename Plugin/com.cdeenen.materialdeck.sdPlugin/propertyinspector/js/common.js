...
function setElements(id) {
    ...
    if (id == 'stats') {
        options = systemData.stats;
        selection = settings.stats ? settings.stats : 'none';
    }
    else if (id == 'ability' || id == 'rollAbility') {
        options = systemData.abilities;
        selection = settings.ability ? settings.ability : options[0]?.value;
    }
    else if (id == 'onClick') {
        options = systemData.onClick;
        selection = settings.onClick ? settings.onClick : 'doNothing';
    }
    else if (id == 'condition') {
        options = systemData.conditions;
        selection = settings.condition ? settings.condition : 'removeAll';
    }
    else if (id == 'skill' || id == 'rollSkill') {
        options = systemData.skills;
        selection = settings.rollSkill ? settings.rollSkill : options[0]?.value;
    }
    else if (id == 'roll') {
        options = systemData.rollTypes;
        selection = settings.roll ? settings.roll : 'ability';
    }
    else if (id == 'save' || id == 'rollSave') {
        options = systemData.saves;
        selection = settings.save ? settings.save : options[0]?.value;
    }
    else if (id == 'inventoryType') {
        options = systemData.itemTypes;
        selection = settings.inventoryType;
        if (selection == undefined) selection = 'any';
    }
    else if (id == 'weaponRollMode') {
        options = systemData.weaponRollModes;
        selection = settings.weaponRollMode;
        if (selection == undefined) selection = 'default';
    }
    else if (id == 'featureType') {
        options = systemData.featureTypes;
        selection = settings.featureType;
        if (selection == undefined) selection = 'any';
    }
    else if (id == 'spellType') {
        options = systemData.spellLevels;
        selection = settings.spellType;
        if (selection == undefined) selection = 'any';
    }
    else if (id == 'spellMode') {
        options = systemData.spellTypes;
        selection = settings.spellMode;
        if (selection == undefined) selection = 'any';
    }
    else if (id == 'attackMode') {
        options = systemData.attackModes;
        selection = settings.attackMode;
        if (selection == undefined) selection = 'chat';
    }
